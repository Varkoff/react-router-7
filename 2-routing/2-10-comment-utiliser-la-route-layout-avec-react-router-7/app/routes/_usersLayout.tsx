			...
			<ul className='flex flex-col gap-4 basis-[400px] flex-1'>
				<div className='flex flex-row gap-2 w-full justify-between'>
					...
					<NavLink
						...
						className='px-4 py-2 rounded-sm bg-emerald-600 text-white'
					>
				...
				{users.map((user) => (
					...
					>
						<NavLink
							className={({ isActive }) =>`text-lg font-mono p-4 shadow-md rounded-lg hover:shadow-lg transition-shadow block ${isActive ? 'bg-blue-800 text-white hover:bg-blue-900' : 'bg-white text-blue-500 hover:text-blue-700'}`}
							to={href('/users/:userSlug', {
								userSlug: user.slug,
							})}
						>
